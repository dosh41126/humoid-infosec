...
class App(customtkinter.CTk):
    ...
    def map_keywords_to_weaviate_classes(self, keywords, context):
        ...
        return mapped_classes    def generate_response(self, user_input):
        """
        Cybersecurity-focused response generator.
        Replaces lottery simulation with an infosec assessment + mitigation planner,
        while preserving policy-gradient exploration and memory/osmosis pipelines.
        """
        try:
            if not user_input:
            ...
            # Optional past interaction context
            past_context = ""
            ...
            # Quantum/physio signals -> biasing factors
            ...
            # ===================== PROMPT (Infosec) =====================
            prompt_parts = [
                "[INFOSPHERE::CYBERSECURITY_ASSESSMENT_MODULE]",
                "------------------------------------------------------------",
                f">> Quantum Security Node ONLINE ∙ Geo({lat}, {lon}) ∙ Weather: {weather}, Temp: {temp}°F",
                f">> Analyst Input: “{cleaned_input}”",
                "",
                "[coherencegain]",
                "Synchronizing homomorphic vector baselines with vault entropy/resistance metrics.",
                "Initializing integrity beacons across binaries, memory pages, and network interfaces.",
                "[/coherencegain]",
                "",
                f"[QuantumStateInfo]\n{quantum_state}",
            ...
            prompt_parts.extend([
                ">>> BEGIN CYBERSECURITY RESPONSE SEQUENCE (x3 Simulation Phases)",
                "------------------------------------------------------------",
                "",
                "[action]",
                "[PHASE 1: SYSTEM & VAULT INTEGRITY]",
                "- Enumerate processes, modules, scheduled tasks, and privileged handles.",
                "- Verify hashes of critical binaries and Python env artifacts against trusted baselines.",
                "- Inspect SecureKeyManager: list versions, validate AAD, nonce, and drift.",
                "- Compute entropy & resistance scores; flag low-entropy or reused secrets.",
                "- Detect signs of prompt/command injection within recent interaction logs.",
                "[/action]",
                "",
                "[coherencegain]",
                "Local deviation fields mapped. Updating rotation vectors and manifold thresholds.",
                "[/coherencegain]",
                "",
                "[action]",
                "[PHASE 2: NETWORK & EXFIL PATTERNS]",
                "- Correlate inbound/outbound flows with process IDs and time windows.",
                "- Detect DNS tunneling, beaconing periodicity, TOR-like signatures, or C2 heuristics.",
                "- Identify lateral movement (ARP/DHCP anomalies, unusual SMB/RDP/SSH auth).",
                "- Compare behavioral embeddings to crystallized safe states (cosine distance).",
                "[/action]",
                "",
                "[coherencegain]",
                "Temporal anomalies attenuated. Cross-linking with prior flagged events.",
                "[/coherencegain]",
                "",
                "[action]",
                "[PHASE 3: MITIGATION & RECOVERY PLAN]",
                "- Classify probable kill-chain stage and compromise vector(s).",
                "- If vault anomaly score ≥ 0.65: rotate active master key and migrate tokens.",
                "- Quarantine suspicious processes, revoke credentials, seal outbound egress rules.",
                "- Regenerate TLS contexts, invalidate sessions, and scrub transient caches.",
                "- Prepare forensics snapshot: signed, timestamped, and encrypted (latest key version).",
                "[/action]",
                "",
                "-------------------------------",
                "[TRANSMISSION: CYBERSECURITY MITIGATION REPORT]",
                "",
                "Infosec Threat Summary:",
                "- Indicators of Compromise (IOCs): <list or 'None'>",
                "- Likely Kill Chain Stage: <Recon|Initial Access|Execution|Persistence|Exfiltration|Impact>",
                "- Threat Confidence: <Low|Medium|High>",
                "- Vault Key Entropy / Resistance: <scores>",
                "- Immediate Containment: <firewall ACLs, proc kill/isolation, credential revocation>",
                "- Forensic Evidence Hash: <SHA-256 or similar>",
                "- Follow-up Tasks: <patching, policy updates, user training, audit schedule>",
                "",
                "# Use only facts observed within [action] phases; label hypotheses explicitly.",
                "# Output must be actionable, minimal, and integrity-signed.",
                "-------------------------------"
            ])
            base_prompt = "\n".join(prompt_parts)
            # ============================================================

            # Candidate exploration via policy gradient
            num_candidates = 4
            candidate_rollouts = []
            ...
            if not candidate_rollouts:
                logger.warning("[PG] No security mitigation candidates produced.")
                self.response_queue.put({'type': 'text', 'data': '[No secure output generated]'})
                return

            best = max(candidate_rollouts, key=lambda c: c["reward"])
            debug_meta = (
                f"[InfosecPredictor] BestReward={best['reward']:.3f} "
            ...
            logger.error(f"[generate_response] Infosec simulation error: {e}")
            self.response_queue.put({'type': 'text', 'data': f'[SecureError] {e}'})
